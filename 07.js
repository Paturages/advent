const input = `wdysq (135) -> sxldvex, wiasj
...
// Part 1
(programList => {
  const programsMap = programList.reduce(
    (obj, [, name]) => Object.assign(obj, { [name]: { name, parents: [] }  }),
    {}
  );
  programList.forEach(([, name, weight, , children]) => {
    if (children && children.split) children.split(', ').forEach(child => programsMap[child].parents.push(name));
  });
  return Object.values(programsMap).find(x => !x.parents.length);
})(input.split('\n').map(x => x.match(/(.+) \((\d+)\)( -> )?(.+)?/)))

// Part 2
(programList => {
  const programsMap = programList.reduce(
    (obj, [, name, weight, , children]) =>
      Object.assign(obj, {
        [name]: {
          name,
          weight: +weight,
  ...
  // Build the whole tree with back and forth object references
  for (name in programsMap) {
    const program = programsMap[name];
    program.children = program.children.map(childName => programsMap[childName]);
    program.children.forEach(child => child.parents.push(program));
  }
  // Calculate tower weights
  for (name in programsMap) {
    const program = programsMap[name];
    program.parents.forEach(function add(parent) {
      parent.towerWeight += program.weight;
      parent.parents.forEach(add);
    });
  }
  // Find the anomaly and calculate the weight differential (delta)
  let head = Object.values(programsMap).find(x => !x.parents.length);
  head = head.children.find((child, i) => i && child.towerWeight != head.children[0].towerWeight) || head.children[0];
  const delta = head.towerWeight - head.parents[0].children[head.parents[0].children[0] == head ? 1 : 0].towerWeight;
  while (head.children.length) {
    const anomaly = head.children.find((child, i) => child.towerWeight != head.children[i ? 0 : 1].towerWeight);
    if (!anomaly) return head.weight - delta;
    else head = anomaly;
  }
