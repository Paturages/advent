const input = `smi inc 781 if epx > -2
...
(instructions =>
  Math.max(...Object.values(
    instructions.reduce((registers, [, name, op, value, condition]) => {
      // Cheat the conditional with eval()! >:)
      const conditionVar = condition.slice(0, condition.indexOf(' '));
      if (!registers[conditionVar]) registers[conditionVar] = 0;
      if (!registers[name]) registers[name] = 0;
      if (eval(`registers.${condition}`)) registers[name] += value * (op == 'dec' ? -1 : 1);
      return registers;
    }, {})
  ))
)(input.split('\n').map(x => x.match(/([a-z]+) (dec|inc) (-?\d+) if (.+)/)))
...
(instructions =>
  instructions.reduce((registers, [, name, op, value, condition]) => {
    ...
    if (!registers[conditionVar]) registers[conditionVar] = 0;
    if (!registers[name]) registers[name] = 0;
    if (eval(`registers.${condition}`)) {
      registers[name] += value * (op == 'dec' ? -1 : 1);
      if (registers[name] > registers.max) registers.max = registers[name];
    }
    return registers;
  }, { max: 0 }).max
)(input.split('\n').map(x => x.match(/([a-z]+) (dec|inc) (-?\d+) if (.+)/)))
